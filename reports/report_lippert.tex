...
\begin{itemize}
  \item Simple model
  \item As iun parkendd: RegressionTree
 \end{itemize}
...
Another challenge was posed by the uninformed use of heroku's free plan. This plan limits available database size and places some restrictions on the web application. In particular, the application runs on so-called dynos. Dynos of the free plan will be stopped when there was no web traffic for 30 minutes. 

This restriction could be circumvented by running a ``keep awake'' script on one of the university server. The sole function of this script is to send web requests in regular intervals so that the dyno is kept awake. Another restriction of the free dynos is a minimum sleeping time of 6 hours per day; the ``keep awake'' script will stop keeping the dyno awake in the night. This poses no significant problem as the parking area of interest is also closed at nigh\footnote{The Libori-Galerie is closed for 5 hours. It is assumed that nothing of interest will happen between 1am and 2am local time.}.
...
\subsection{Comparison between methodologies}
The original predictions were achieved by training a RegressionTree
...
It was chosen to develop the web application in Scala using the Play! framework, mainly because of familiarity with the language and the easy access to machine learning libraries.
...
\subsubsection{Frameworks}\label{sec:frameworks}
